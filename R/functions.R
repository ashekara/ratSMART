...
# load(file.path(.libPaths()[1],"ratbrain","data","atlasIndex.RData"))
...
get_count <- function(dataset, roi = c('MO', 'TH')){
  out <- unlist(lapply(roi, function(x)sum(dataset$acronym %in% ratbrain::get.sub.structure(x))))
...
get_all_children <- function (rois, children = c()) {
  for (l in 1:length(rois)){
    # Get child regions
    new_children <- ratbrain::get.acronym.child(rois[l])
...
get_hemi_cnts <- function(dataset, roi = c('MO', 'TH'), hemi= "Right"){
  if (hemi == "Right") {
    out <- unlist(lapply(roi, function(x)sum(dataset$acronym %in% ratbrain::get.sub.structure(x) * dataset$right.hemisphere)))
  } else if (hemi == "Left") {
    out <- unlist(lapply(roi, function(x)sum(dataset$acronym %in% ratbrain::get.sub.structure(x) * !dataset$right.hemisphere)))
  }
  roi.data <- data.frame(acronym = roi, cell.count = out)
