...
#' @param numthresh (optional, default = 8) Original argument from [ratbrain::segment()].
#' @param downsample (optional, default = 0.25) Original argument from [ratbrain::segment()].
#' @param post (optional, default = NULL) Original argument from [ratbrain::segment()].
#' @param pre (optional, default = NULL) Original argument from [ratbrain::segment()].
#' @param get.contour (optional, default = FALSE) Original argument from [ratbrain::segment()].
#' @param channel (optional, default = 0) Original argument from [ratbrain::segment()].
...
#' @seealso See also the [ratbrain::segment()] function.
#' @export
#' @md
...
seg_loop <- function(setup, filter, numthresh = 8, downsample = 0.25, post = NULL,
  ...
  if (length(setup) > 9) {
    ...
    # If not ratbrain dataset, segment all the images in setup$regi_z
    seg_z <- setup$regi_z
  }
  ...
  for (s in 1:length(seg_z)) {
   ...
   segs$segmentations[[s]] <- ratbrain::segment(im_path, numthresh = 8, downsample = downsample, filter = filter,
