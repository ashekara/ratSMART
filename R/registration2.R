#' @title Console user interface built on top of [registration()] function from the ratbrain package.
#' @description Allows you to alter correspondance points with user interface with the console
#' This uses the original [registration()] function from ratbrain and retains the same arguments.
...
#' @seealso See also [ratbrain::registration()] for the original registration function from ratbrain.
#' @export
#' @md
...
registration2 <- function(input, coordinate = NULL, plane = "coronal",
  ...
  regi <- ratbrain::registration(input, coordinate = coordinate, plane = plane,
  ...
  while (!regi_done ) {
      ...
      while (corr_done!= "A" & corr_done!= "a" & corr_done!= "C" & corr_done!= "c" & corr_done!= "R" & corr_done !=
        ...
        if (corr_done == "Z" | corr_done == "z") {
          ...
          quartz(width, height)
          regi_new <- ratbrain::registration(input, coordinate = coordinate, plane = plane,
                                               right.hemisphere = right.hemisphere, interpolation = interpolation, intrp.param = intrp.param,
                                               brain.threshold = brain.threshold, blurring = blurring,
          ...
          if (corr_done == "A" | corr_done == "a") {
            ...
            regi_new <- ratbrain::add.corrpoints(regi, pts)
            ...
            regi_new <- ratbrain::change.corrpoints(regi, pts)
            ...
            regi_new <- ratbrain::remove.corrpoints(regi, pts)
