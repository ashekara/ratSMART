...
#' @param autoloop (optional, default = FALSE) Run autoloop to save ratbrain's first pass at
...
regi_loop <- function(setup, filter = NULL, regis = NULL, plane = "coronal", closewindow = TRUE,
  ...
  if (autoloop) {
    ...
    for (s in 1:length(setup$regi_z)){
      ...
      # Register image
      quartz(width, height)
      regis[[s]] <<- ratbrain::registration(setup$image_paths$regi_paths[imnum], AP,
