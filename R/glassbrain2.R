...
#' @description A modified version of [ratbrain::glassbrain()] that
...
#' @param dim (default = c(720, 1080))  Original argument from [ratbrain::glassbrain()].
#' @param device (default = TRUE) Original argument from [ratbrain::glassbrain()].
#' @param col (default = "region") Original argument from [ratbrain::glassbrain()].
#' @param cex (default = 0.5) Graphical parameter for text size.
#' @param hemisphere (default = "right") Original argument from [ratbrain::glassbrain()].
#' @param spheres (default = FALSE) Original argument from [ratbrain::glassbrain()].
#' @param alpha (default = 1) Original argument from [ratbrain::glassbrain()].
#' @param laterality (default = TRUE) Original argument from [ratbrain::glassbrain()].
#' @param plane (default = "coronal") Original argument from [ratbrain::glassbrain()].
#' @param jitter (default = FALSE) If TRUE, add jitter in z-axis to fill space between z-planes.
#' @seealso See also the function [ratbrain::glassbrain()] from the ratbrain package.
#' @export
#' @md
...
glassbrain2 <- function(dataset, high.res = "OFF", dim = c(720, 1080), device = TRUE,
  ...
  data(glasbrain, package = "ratbrain", envir = environment())
  ...
  if (spheres) {
    rgl::spheres3d(ratbrain::paxTOallen(dataset$AP) - 530/2 + smp.AP, -dataset$DV *
                1000/25 - 320/2, dataset$ML * 1000/25 + smp.ML, col = color,
              radius = cex, alpha = alpha)
  }
  else {
    rgl::points3d(ratbrain::paxTOallen(dataset$AP) - 530/2 + smp.AP, (-dataset$DV *
