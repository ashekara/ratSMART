...
#'   registration data.
#' @seealso See also [ratbrain::schematic.plot()].
#' @export
#' @md
...
forward_warp <- function(setup, segs, regis,
  ...
  for (n in 1:length(segs$seg_z)) {
    ...
    data <- ratbrain::inspect.registration(regi, seg, soma = TRUE,
    ...
    # Save schematic plot
    if(saveschematic){
      ratbrain::schematic.plot(dataset = data, plane = plane, save.plots = FALSE, title = title, mm.grid = mm.grid,
